...
OBJECTS = ['A', 'B', 'C']
NUM_EPOCHS = 10000
INPUT_FEATURES = ['shortwave_radiation',
                  'pressure', 'relative_humidity', 'temperature', 'rain', 'month', 'day_of_week', 'hour']
for object in OBJECTS:
    results_plot_title = f'Ražošanas prognozes pret patiesajām vērtībām {object} objektam - GRU'
    ...
    loss_plot_title = f'GRU modeļa apmācības un validācijas zaudējumi {object} objektam'
    loss_save_path = f'plots/GRU-{object}-loss.png'
    ...
    model = GRU(input_dim=X_train.shape[1],
                hidden_dim=64, output_dim=1, num_layers=2)
    model = model.to('cpu')
    ...
    early_stopping = EarlyStopping(
        object_name=object, patience=50, min_delta=0.0001, model_type=ModelType.NBEATS)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if early_stopping.should_stop():
            ...
            break
    loss_plot = PlotLoss("GRU", object_name=object, title=loss_plot_title, save_path=loss_save_path,
    ...
    results_plot = PlotPredictions("GRU", object_name=object, title=results_plot_title, save_path=results_save_path, ground_truth=ground_truth_df,
